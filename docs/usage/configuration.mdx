...
This means Tach will expect that your Python imports are resolved relative to the directory in which `tach.yml` exists.
...
in your `tach.yml`, or use [`tach mod`](commands#tach-mod) and mark the `backend` folder as the only source root.
...
in your `tach.yml`, or use [`tach mod`](commands#tach-mod) and mark the same folders as source root.

In `tach.yml`, each entry in `source_roots` is interpreted as a relative path from the project root.
...
Adding the top level module name to the `exclude` key will allow all usages of the corresponding module.

...
This is useful to allow multiple developers or teams to independently own and maintain their modules and interfaces.

### Modules

Within a `tach.domain.toml` file, you can define [modules](#modules) similarly to how you would in `tach.toml`.
The key difference is that paths are relative to the location of the domain by default, and there is a special syntax for describing the parent folder itself.
...
Tach domain configuration files enable smooth integration with `CODEOWNERS`:

...
> **Note:** Make sure you have the Rust compiler installed. This package requires Rust and Cargo to compile extensions.

### Step 1: Create and Activate a Virtual Environment

Before running `make install`, it's recommended to create and activate a Python virtual environment.
...
On some systems, the `make install` command may fail with an error stating that the `python` command is not found. This happens because some systems only provide `python3` and do not include a `python` symlink.
...
#### Linux Fix:
On Linux, you may need to install the `python-is-python3` package or create a symlink manually:

```bash
sudo apt install python-is-python3  # For Debian/Ubuntu
```

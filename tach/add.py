...
from tach.parsing import dump_project_config_to_yaml, parse_config, toml_config_exists
...
def add_packages(paths: set[str], tags: Optional[set[str]]) -> Iterable[str]:
    ...
    if not project_root:
        raise TachError(
            f"Could not find project root ({CONFIG_FILE_NAME}.yml or {TOML_CONFIG_FILE_NAME})."
        )

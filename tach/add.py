...
from tach.core import FullConfig
...
def build_package(path: str, tags: Optional[set[str]]) -> Optional[str]:
    ...
    # TODO: option to write TOML? write both?
...
def update_project_config(root: str, config: FullConfig, tags: set[str]):
    ...
    try:
        ...
        # TODO: update each YML and TOML if each exists
...
def add_packages(paths: set[str], tags: Optional[set[str]]) -> Iterable[str]:
    project_root = fs.find_project_config_yml_root(path=".")
    if not project_root:
    ...
    warning = update_project_config(
        root=project_root, config=config, tags=tags if tags else new_tags
    )
    if warning:
        return [warning]
    return []
